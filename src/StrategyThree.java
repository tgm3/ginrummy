...
public class StrategyThree implements PlayerStrategy {
    ...
    public static ArrayList<Card> deadwood = new ArrayList<Card>();
    ...
    public boolean willTakeTopDiscard(Card card){
        ...
        for (SetMeld setMeld : setMelds) {
            if (setMeld.canAppendCard(card)) {
                return true;
            }
        }
        for (RunMeld runMeld : runMelds) {
            if (runMeld.canAppendCard(card)) {
                return true;
            }
        }
        double averageValueOfDeadwood = 0;
        for (Card aDeadwood : deadwood) {
            averageValueOfDeadwood += aDeadwood.getPointValue();
        }
        averageValueOfDeadwood /= deadwood.size();
        return card.getPointValue() <= (int) averageValueOfDeadwood;
    ...
    public Card drawAndDiscard(Card drawnCard){
        ...
        for (Card aDeadwood : deadwood) {
            if (aDeadwood.getPointValue() > throwaway.getPointValue()) {
                throwaway = aDeadwood;
            }
        }
    ...
    public boolean knock(){
        int totalDeadwood = 0;
        for (Card aDeadwood : deadwood) {
            totalDeadwood += aDeadwood.getPointValue();
        }
        return totalDeadwood <= 10;
    }
    ...
    public List<Meld> getMelds(){
        ...
        melds.addAll(setMelds);
        melds.addAll(runMelds);
        return melds;
